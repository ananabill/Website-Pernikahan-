...
// Update tombol jika audio selesai
audio.addEventListener('ended', () => {
  playBtn.textContent = '🎵 Putar Musik';
  isPlaying = false;
});

// Galeri slider (2 foto)
...
function renderRsvp(){
  ...
  items.slice().reverse().forEach(({nama,status,jumlah,ts})=>{
    ...
    const date = new Date(ts).toLocaleString('id-ID',{dateStyle:'medium', timeStyle:'short'});
    div.innerHTML = `<strong>${nama}</strong> • ${status}${jumlah?` (${jumlah} orang)`:''} • <span class="muted">${date}</span>`;
...
// filepath: c:\xampp\htdocs\Website Pernikahan\script.js
const hapusUcapanBtn = document.getElementById('hapusUcapanBtn');

hapusUcapanBtn.addEventListener('click', () => {
  localStorage.removeItem('doa_ucapan_v2');
  ucapanList.innerHTML = ''; // Kosongkan tampilan
});